...
import { PROFILE_UUID } from './update-database-permissions.const';
import { ColumnCheck, TableOperation } from './update-database-permissions.types';
...
export const ROW_PERMISSIONS: { [permission in PermissionName]: RowPermission[] | null } = {
	...
	// This permission protects the frontend route.
	// The database rows are protected by VIEW_ANY_PUBLISHED_ITEMS, VIEW_ANY_UNPUBLISHED_ITEMS
	VIEW_ITEMS_OVERVIEW: null,
	...
	EDIT_TRANSLATIONS: [
		{
			...
			// Only update existing frontend or backend translations row.
			// New site variables will need to be added by the admin user and not by the avo user role
			operation: 'update',
		},
	],
	EDIT_USER_GROUPS: [
