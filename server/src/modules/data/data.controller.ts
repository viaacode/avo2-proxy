...
export default class DataController {
	...
	private static filterAppMetaData(response: any): { items: any[]; errors: string[] } {
		...
		if (items && items.length) {
			...
			response.data.app_item_meta = compact(
				(items || []).map((item: Avo.Item.Item) => {
					if (item.is_deleted) {
						errors.push('DELETED');
						return null;
					}
	...
	private static filterAssignments(
		response: any,
		user: Avo.User.User | null
	): { assignments: any[]; errors: string[] } {
		const assignments = get(response, 'data.assignments');

		if (assignments && assignments.length) {
			const errors: string[] = [];
			response.data.assignments = compact(
				(assignments || []).map((assignment: Partial<Avo.Assignment.Assignment>) => {
					const isOwner =
						assignment.owner_profile_id &&
						user &&
						user.profile &&
						user.uid &&
						assignment.owner_profile_id === user.profile.id;
					...
					if (
						...
						new Date(assignment.available_at).getTime() < new Date().getTime() &&
						!isOwner
					) {
					...
					// Owners can always see their assignments even if they are outside of the available_at, deadline_at time interval
					if (
						new Date().getTime() > new Date(assignment.deadline_at || 0).getTime() &&
						!isOwner
					) {
